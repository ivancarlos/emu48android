...
DWORD dwGPCycles = 123 * 3; // g+ cpu cycles in interval    // CdB for HP: add
                            // apples display management
DWORD dwG2Cycles = 123 * 2; // gII cpu cycles in interval    // CdB for HP: add
                            // apples display management
...
static __inline VOID AdjustSpeed(VOID) // adjust emulation speed
{
  ...
  if (bEnableSlow && (bCpuSlow || bKeySlow || bSoundSlow || nOpcSlow > 0)) {
    ...
    EnterCriticalSection(&csSlowLock);
    {
      ...
      if ((dwCycles = (DWORD)(Chipset.cycles & 0xFFFFFFFF) - dwOldCyc) >=
          dwT2Cycles) {
        LARGE_INTEGER lAct;
        do {
        ...
        // ticks elapsed or negative number (workaround for
        // QueryPerformanceCounter() in Win2k)
...
SetT2Cycles(VOID) // set device specific cpu cycles in interval
{
...
VOID UpdateKdnBit(VOID) // update KDN bit
{
  if (Chipset.intk && (Chipset.IORam[TIMER2_CTRL] & RUN) != 0 &&
      (DWORD)(Chipset.cycles & 0xFFFFFFFF) - Chipset.dwKdnCycles >
          dwT2Cycles * 16)
...
UINT SwitchToState(UINT nNewState) {
  ...
  case SM_SLEEP: // Sleep
    ...
    case SM_RUN: // -> Run
      ...
      // don't enter opcode loop on interrupt request
      bInterrupt =
          (nDbgState == DBG_OFF) && (Chipset.Shutdn || Chipset.SoftInt);
      ResumeDebugger();
  ...
  while (nNextState == SM_RUN) {
    if (nState != SM_RUN) {
      ...
      if ((Chipset.IORam[CARDCTL] & ECDT) == 0 &&
        ...
        BOOL bNINT2 =
            Chipset.IORam[SRQ1] == 0 && (Chipset.IORam[SRQ2] & LSRQ) == 0;
      ...
      RomSwitch(
          Chipset.Bank_FF); // select HP49G ROM bank and update memory mapping
      ...
      StartDisplay((BYTE)(((Chipset.IORam[LINECOUNT + 1] << 4) |
                           Chipset.IORam[LINECOUNT]) &
                          0x3F));
    ...
    if (Chipset.Shutdn &&
        !(nDbgState == DBG_STEPINTO || nDbgState == DBG_STEPOVER)) {
